<script lang="ts">
	...
	let showCheckoutField = $derived.by(() => {
		return formFunc().then((value) => value);
	});

	const formFunc = async () => {
		let allowForm: boolean = false;

		if (returnDate !== undefined && deliveryDate !== undefined) {
			const res = await fetch('/panel/checkout/server', {
				method: 'POST',
				headers: {
				...
				body: JSON.stringify({
					...
					items: untrack(() => $formData.items),
					token: untrack(() => data.token)
				})
			});

			if (res.ok) {
				const data: CheckAvailabilityResponse & { allowCheckout: boolean } = await res.json();

				console.log('Boolean: ', data.allowCheckout);
				if (data.allowCheckout === false) {
					toast.error(data.resBody.result.message);
				}
				console.log('Before', allowForm);
				allowForm = data.allowCheckout;
			}
		}
					...
					<Popover.Trigger {...props}>
						{#snippet child({ props })}
							<Button
								...
								{...props}
							>
								{returnDate ? df.format(returnDate.toDate(getLocalTimeZone())) : 'Изберете дата'}
								<CalendarIcon class="ml-auto size-4 opacity-50" />
							</Button>
						{/snippet}
					</Popover.Trigger>
		...
		<section aria-labelledby="order-heading" class="bg-gray-50 px-4 py-6 sm:px-6 lg:hidden">
			<div class="mx-auto max-w-lg">
				...
				>
					...
					<span class="text-base"
						>{$cartStats.total.toLocaleString('bg-BG', {
							style: 'currency',
							currency: 'BGN'
						})}</span
					>
